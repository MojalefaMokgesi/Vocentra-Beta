...
namespace Vocentra.Controllers
{
    public class JobController : Controller
    {
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            // Determine whether the job is publicly visible (paid + active)
            var isPublic = job.IsPaid && string.Equals(job.Status, "Active", StringComparison.OrdinalIgnoreCase);

            // Allow owner and admins to view the job regardless of public status.
            var userId = _userManager.GetUserId(User);
            var isAdmin = User.IsInRole("Admin");

            ViewBag.IsPublic = isPublic;
            ViewBag.IsOwner = isAdmin || job.OwnerUserId == userId;

            // Do not block viewing; show the job page but hide/disable apply actions for non-public jobs.
            return View(job);
        }
