...
namespace Vocentra.Services
{
    public class PayFastService
    {
        ...
        string itemName)
        {
            // PayFast expects amount as "0.00" using dot decimal
        ...
        {
            var received = itnData.TryGetValue("signature", out var s) ? s : string.Empty;
        ...
        public async Task<bool> ValidateItnAsync(IDictionary<string, string> itnData, CancellationToken ct = default)
        {
            ...
            using var resp = await client.PostAsync(ValidateUrl, new FormUrlEncodedContent(itnData), ct);
